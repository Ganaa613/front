...
const CategoryPage: React.FC = () => {
  ...
  async function handleDelete(id: string) {
    try {
      await httpService.delete(`/resource-catalog/candidate/${id}`);
      actionRef.current?.reload();
      notifyService.success(`Candidate succcessfully deleted`);
    } catch (error: any) {
  ...
  const request = async (params: any) => {
    try {
      ...
      const res = await httpService.get<list.IResponse<resource.Candidate>>(
  ...
  const columns: ProColumns<resource.Candidate>[] = [
    ...
    },
    {
      ...
      render: (record: any) => (
        <AppTableItemControll
          viewAction={() => navigate(`/resource/candidate/${record.id}`)}
      ...
      <AppTable
        ...
        toolBarRender={() => [
          <Button
            key="add"
            type="primary"
            onClick={() => navigate("/resource/candidate/create")}
          >
            Add
          </Button>,
        ]}
