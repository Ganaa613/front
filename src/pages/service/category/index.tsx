...
const CategoryPage: React.FC = () => {
  ...
  async function handleDelete(id: string) {
    try {
      await httpService.delete(`/service-catalog/category/${id}`);
  ...
  const request = async (params: any) => {
    try {
      ...
      const res = await httpService.get<list.IResponse<service.Category>>(
        `/service-catalog/category?${queryParams}`
      );
  ...
  const columns: ProColumns<service.Category>[] = [
    ...
    },
    {
      ...
      render: (record: any) => (
        <AppTableItemControll
          viewAction={() => navigate(`/service/category/${record.id}`)}
