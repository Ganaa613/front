...
const CatalogPage: React.FC = () => {
  ...
  async function handleDelete(id: string) {
    try {
      await httpService.delete(`/service-catalog/catalog/${id}`);
  ...
  const request = async (params: any) => {
    try {
      ...
      const res = await httpService.get<list.IResponse<service.Catalog>>(
        `/service-catalog/catalog?${queryParams}`
      );
  ...
  const columns: ProColumns<service.Catalog>[] = [
    ...
    },
    {
      ...
      render: (record: any) => (
        <AppTableItemControll
          viewAction={() => navigate(`/service/catalog/${record.id}`)}
      ...
      <AppTable
        ...
        toolBarRender={() => [
          <Button
            key="add"
            type="primary"
            onClick={() => navigate("/service/catalog/create")}
          >
            Add
          </Button>,
        ]}
