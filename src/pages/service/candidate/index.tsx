...
const CandidatePage: React.FC = () => {
  ...
  async function handleDelete(id: string) {
    try {
      await httpService.delete(`/service-catalog/candidate/${id}`);
  ...
  const request = async (params: any) => {
    try {
      ...
      const res = await httpService.get<list.IResponse<service.Candidate>>(
  ...
  const columns: ProColumns<service.Candidate>[] = [
    ...
    },
    {
      ...
      render: (record: any) => (
        <AppTableItemControll
          viewAction={() => navigate(`/service/candidate/${record.id}`)}
      ...
      <AppTable
        ...
        toolBarRender={() => [
          <Button
            key="add"
            type="primary"
            onClick={() => navigate("/service/candidate/create")}
          >
            Add
          </Button>,
        ]}
