...
export default function ProductSpecificationPage() {
  ...
  const columns: ProColumns<service.Specification>[] = [
    {
      key: "id",
      title: "Id",
      dataIndex: "id",
    ...
    },
    {
      ...
      render: (record: any) => (
        <AppTableItemControll
          viewAction={() => navigate(`/service/specification/${record.id}`)}
  ...
  const request = async (params: any) => {
    try {
      ...
      const res = await httpService.get<
        list.IResponse<service.Specification[]>
  ...
  async function handleDelete(item: service.Specification) {
    try {
      await httpService.delete(`/service-catalog/specification/${item.id}`);
      ...
      <div>
        <AppTable
          ...
          toolBarRender={() => [
            <Button
              type="primary"
              onClick={() => navigate("/service/specification/create")}
            >
              Add
            </Button>,
          ]}
