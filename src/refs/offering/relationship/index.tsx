...
const Component: React.FC<ComponentProps> = ({
  ...
  const [dataSource, setDataSource] =
    useState<product.ProductOfferingRelationship[]>(data);
  ...
  const editItem = (item: product.ProductOfferingRelationship) => {
    setItem(undefined);
    setDataSource((state) => {
      return state.map((e) => {
        if (e.id === item.id) {
          item.validFor = getValidForToObject(item.validFor) ?? {};
          return item;
        }
        return e;
      });
  ...
  const onChange = (item: product.ProductOfferingRelationship) => {
